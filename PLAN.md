...
| hyprland | 🔄 **In progress (2026-09-24), user decided to continue through all of it.** Terra dropped Hyprland core, but that turned out to matter less than feared — most of the real gaps have Fedora rawhide references (unbranched to EPEL, same pattern throughout this repo). Verified/built so far: `hyprwayland-scanner`, `hyprutils` (bumped 0.7.1→0.14.2, real SOVERSION break caught: 6→13), `hyprlang` (bumped 0.6.4→0.6.8, SOVERSION unchanged), `hyprcursor`, `hyprgraphics` (bumped 0.1.5→0.5.1, SOVERSION 0→4), `libspng`, `libxkbcommon` (1.7.0→1.13.1, **system-wide replacement** — successfully upgraded on `durin` itself with zero breakage after Zoom, the one real conflicting consumer, was removed), `muparser` (no distro reference anywhere — written from scratch, informed by openSUSE's spec), `lua` (assumed from-scratch needed, turned out Fedora already tracks 5.5.1 under the plain `lua` name — **real caveat found**: this collides with el10's own built-in `lua-libs` 5.4, which `ibus-libpinyin`/`brlapi` hard-depend on; did NOT force this onto `durin`, verified via mock only — needs a real packaging decision before going into the actual COPR, see SETUP.md Step 17), `rust-calloop`, `rust-input` (both needed by `mir`'s Rust component, discovered via `mir`'s dependency chain, not hyprland's). Remaining: `aquamarine` and `hyprland` itself, both genuinely from-scratch (no reference spec anywhere), though their own dependency lists are otherwise fully tractable — checked directly, no toolchain gaps (CMake 3.30+, C++26 both work fine on el10). | Fork `aquamarine`, then `hyprland` itself, once all lib bumps are verified in mock |
| miraclewm | ⚠️ **Correction (2026-09-24): this row was wrong.** Initial research checked the wrong package names (`miraclewm`, `mir-libs`, `miral`, `mircommon` as separate dist-git repos) and concluded the whole Mir toolkit was unpackaged anywhere — user correctly pushed back. The real names are `mir` (a single spec building `miral`/`mircommon`/`mirserver`/`mirwayland`/etc. as subpackages) and `miracle-wm` (hyphenated), **both real, actively-maintained Fedora packages** (`mir` updated Sep 21 2026, `miracle-wm` Sep 21 2026) — there's even an official Fedora Spin for it. Full dependency sweep against el10 found nearly everything already present, including niche ones (`wlcs`, `glm-devel`, `gflags-devel`); only 2 test-only deps missing (`umockdev`, `python3-dbusmock` — both %check-only, safely skippable). Forked both specs; `mir` build in progress. | Mock-build `mir` (large, in progress), then `miracle-wm` |
