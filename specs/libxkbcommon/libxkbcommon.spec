# ---------------------------------------------------------------------------
# Forked from Fedora's rawhide dist-git for dank-ws-copr, unmodified:
#   https://src.fedoraproject.org/rpms/libxkbcommon/raw/rawhide/f/libxkbcommon.spec
# Adapted 2026-09-24. This REPLACES el10's own `libxkbcommon` (currently 1.7.0) - not a side-by-side
# compat package, since hyprland/mir and friends reference it by its plain unversioned pkgconfig
# name. Needed because hyprland requires xkbcommon >=1.11.0. Per the standing decision to bump
# shared libraries system-wide when a versioned side-by-side package isn't feasible (same choice
# made for mangowm's pixman/xkbcommon situation - see PLAN.md's mangowm blocker entry). All
# BuildRequires confirmed present on el10 before forking; static Release already (no %%autorelease/
# %%evr issue).
