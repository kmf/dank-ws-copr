...
#   https://src.fedoraproject.org/rpms/greetd/raw/rawhide/f/greetd.spec
# Adapted 2026-09-24. Confirmed via Fedora dist-git branch listing that greetd is genuinely just
# unbranched for EPEL (spec exists at rawhide/f41/f42, 404s at epel9/epel10) - not broken or
# unported, just nobody has requested the branch. See SETUP.md Step 10 for that check and Step 11
# for the actual build attempt. `dms-greeter`/`dms-greeter-git` (avengemedia COPR) are hard-blocked
# on this: `nothing provides greetd`.
#
# Every non-crate BuildRequires (cargo-rpm-macros, make, scdoc, sed, systemd-rpm-macros,
# selinux-policy-devel) is already on el10. Of its Rust crate dependencies, only two
# (`pam-sys`, `enquote`) were missing - both forked separately into specs/rust-pam-sys/ and
# specs/rust-enquote/ from Fedora rawhide too (also genuinely just unbranched, same pattern).
#
...
#
# STATUS: not yet mock-built. See SETUP.md Step 11 for build attempt/results once run.
# ---------------------------------------------------------------------------
