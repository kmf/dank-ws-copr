# ---------------------------------------------------------------------------
# Forked from Fedora's rawhide dist-git for dank-ws-copr, unmodified (all sources/patches
# vendored directly from Fedora's dist-git, none on lookaside - fetched via raw file URLs):
#   https://src.fedoraproject.org/rpms/lua/raw/rawhide/f/lua.spec
# Adapted 2026-09-24. Initially assumed this needed a from-scratch spec (checked `lua5.5`/`lua-5.5`/
# `lua55` as separate package names, all 404) - Fedora's actual package is simply named `lua`
# (unversioned) and had *already* been bumped to track 5.5.1 by the time of this check. Not
# branched to epel9/epel10 (404 on both), same unbranched pattern as everything else. Needed
# because `hyprland`'s CMakeLists.txt does `pkg_search_module(LUA REQUIRED ... lua>=5.5 lua<5.6)` -
# specifically Lua 5.5, no fallback to 5.4.
#
