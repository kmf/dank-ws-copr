# ---------------------------------------------------------------------------
# Forked/adapted from Fedora's rawhide dist-git for dank-ws-copr, unmodified:
#   https://src.fedoraproject.org/rpms/python-dbusmock/raw/rawhide/f/python-dbusmock.spec
# Adapted 2026-09-24. Not branched to epel9/epel10 (404 on both). Real second-round finding: `mir`
# needs `python3-dbusmock` genuinely at CMake *configure* time too (a Python import check, not just
# test-execution) - discovered via a real failed mir build (`ModuleNotFoundError: No module named
# 'dbusmock'`) after wrongly assuming (based on Fedora's own "# For the tests" comment) that this
# one, unlike umockdev, was safely conditional on run_tests. It isn't. Dist-git naming caught us
# out too: it's `python-dbusmock` (not `python3-dbusmock`, which is only the built binary RPM name)
# - the earlier "doesn't exist in Fedora at all" claim was from checking the wrong dist-git path.
# All build deps confirmed present on el10; static Release already (no %%autorelease/%%evr issue).
